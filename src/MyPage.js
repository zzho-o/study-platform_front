...
export default function MyPage() {
  ...
  React.useEffect(() => {
    const tmp = pr.filter((item) =>
      item.toUpperCase().includes(st.toUpperCase())
    );
    setSearch(tmp);
  }, [st]);
  return (
    ...
    >
      ...
      >
        <Avatar sx={{ width: "150px", height: "150px" }} src={avt}></Avatar>
      </Box>
      <TextField
        noBorder
        placeholder="닉네임"
        ...
        {search
          ? search.map((item) => (
              <Fab
                ...
                name={item}
                onClice={(e) =>
                  tech.include(e.currentTarget.name)
                    ? null
                    : setTech([...tech, e.currentTarget.name])
                }
              >
                {`${item}`}
              </Fab>
      ...
      <Box>
        {tech ? tech.map((item) => <Chip sx={{ mr: 1 }} label={item} />) : null}
      </Box>
    </Box>
  );
}
