...
import { isLogined, ModalOpen, isLogOuted } from "../../redux/slices/userSlice";
...
function AppAppBar() {
  ...
  const modalOpen = useSelector((state) => state.userReducer.modalOpen);
        ...
        <Toolbar sx={{ justifyContent: "space-between" }}>
          ...
          {isLogIn ? (
            ...
            >
              ...
              <LogoutIcon
                sx={{ ml: 3 }}
                onClick={() => dispatch(isLogOuted(false))}
              />
            </Box>
          ) : (
            <Box sx={{ flex: 1, display: "flex", justifyContent: "flex-end" }}>
              {
                ...
                <LoginArea onClick={() => dispatch(ModalOpen(true))}>
                  로그인 / 회원가입
                </LoginArea>
              }
            </Box>
